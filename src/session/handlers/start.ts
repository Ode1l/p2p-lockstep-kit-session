...
export const start: CommandListener = (command) => {
  ...
  if (command.origin === "local") {
    ...
    if (!state.canAction("self", "START", selfNext) || !state.canAction("peer", "PEER_START", peerNext)) {
      return;
    }
  ...
  const starterSide = resolveStarterFromPayload(
    (command.payload as { starter?: "sender" | "receiver" } | undefined)?.starter,
  );
  const selfNext = starterSide === "self" ? "my_turn" : "peer_turn";
  const peerNext = starterSide === "self" ? "peer_turn" : "my_turn";
  if (!state.canAction("self", "PEER_START", selfNext) || !state.canAction("peer", "START", peerNext)) {
    return;
  }
